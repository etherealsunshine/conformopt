"""Corrected tmol audit for all Probe 4c endpoints, detached and checkpointed."""
...
APP = modal.App("probe4c12-endpoint-tmol-audit")
...
IMAGE = (
    ...
    .add_local_dir(
        ROOT / "probe4c12_results" / "endpoint_audit",
        remote_path="/audit",
        copy=True,
    )
)
...
def score_all() -> dict:
    ...
    output_dir = Path("/outputs/probe4c12_results/endpoint_audit")
    ...
    for site_index, site in enumerate(inputs["sites"]):
        ...
        def energy(candidate) -> float:
            # Reparse the heavy-atom candidate so tmol rebuilds hydrogens at
            # the new conformation.  This avoids the orphaned-H artifact found
            # and corrected during the Probe 4b physical audit.
    ...
    summary = {
        ...
        "endpoints_scored": len(rows), "sites": len(inputs["sites"]),
    }
