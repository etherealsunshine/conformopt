...
def main() -> None:
    ...
    for site_index, site in enumerate(inputs["sites"]):
        ...
        def context(assignment: str):
            if assignment in contexts:
                return contexts[assignment]
            base_key = (
                f"base_pdb_{assignment}"
                if assignment in {"A", "B"} and f"base_pdb_{assignment}" in site
                else "base_pdb"
            )
            base_pdb = args.input_root / site[base_key]
            ...
            result = (
                base_pdb.read_text().splitlines(),
                scorer,
                {},
                base_key,
            )
            contexts[assignment] = result
            return result

        def energy(candidate: list[list[float]], assignment: str) -> float:
            base_lines, scorer, energy_cache, _base_key = context(assignment)
        ...
        energy_a = energy(site["A"], "A")
        energy_b = energy(site["B"], "B")
        if args.calibrate_only:
            random_energy = energy(site["random_rotamers"][0], "other")
            calibration_rows.append(
                {
                    "site": site["site"],
                    "tmol_A": energy_a,
                    "tmol_B": energy_b,
                    "tmol_environment_rule": site.get(
                        "tmol_environment_rule", "legacy_shared_base"
                    ),
        ...
        random_energies = [
            energy(candidate, "other") for candidate in site["random_rotamers"]
        ]
        site_rows = []
        for candidate in site["candidates"]:
            assignment = candidate.get("assignment", "other")
            site_rows.append(
                {
                    ...
                    "tmol_energy": energy(candidate["coordinates"], assignment),
                    "tmol_A": energy_a,
                    "tmol_B": energy_b,
                    "tmol_environment_assignment": assignment,
                    "tmol_environment_rule": site.get(
                        "tmol_environment_rule", "legacy_shared_base"
                    ),
