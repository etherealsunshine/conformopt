...
def build_context_for_runner(
    ...
    *,
    ...
    if monomer_root is None:
        configured_root = os.environ.get("PHENIX_ROOT") or os.environ.get("PHENIX")
        roots = [Path(configured_root)] if configured_root else []
        candidates = []
        for phenix_root in roots:
            candidates.extend((
        ...
        monomer_root = next((path for path in candidates if path.is_dir()), None)
        if monomer_root is None:
            raise FileNotFoundError(
                "could not locate the Phenix monomer library; set PHENIX_ROOT "
                "or pass monomer_root when building a clash context"
            )
