...
OUTPUT=${OUTPUT:-/home/dev/qfit_unet_data/density_denoiser/heldout_three_four_chi_200step_synthetic_v1}
FRAME=${FRAME:-crystal}
...
payload["subset_rule"] = (
    "predeclared synthetic four-chi 200-step schedule ablation; "
...
mkdir -p \
  "$OUTPUT/logs" "$OUTPUT/pids" "$OUTPUT/selections" "$OUTPUT/analysis" \
...
write_status occupancy_sensitivity
"$PYTHON" -m density_denoiser.recompute_strict_occupancy_tolerances \
  --ensemble-table "$OUTPUT/audit/original2/ensemble_strict_audit.csv" \
  --ensemble-table "$OUTPUT/audit/expanded1/ensemble_strict_audit.csv" \
  --tolerance 0.20 \
  --tolerance 0.10 \
  --tolerance 0.05 \
